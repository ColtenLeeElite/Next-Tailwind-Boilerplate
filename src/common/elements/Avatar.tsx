...
const Avatar: FC<AvatarProps> = ({
  ...
  const colorClass = isHuman ? 'border-neon-100' : 'border-orange'
  ...
  return (
    <div className="relative z-10 inline-block h-fit">
      ...
      {badgeIcon === 'icon' ? (
        <span
          className={`flex justify-center items-center absolute bottom-0 right-0 h-${badgeHeight} w-${badgeWidth} rounded-full ring-1 ring-[#0C0D10] bg-[#36393F] text-white hover:cursor-pointer`}
        >
          <input
