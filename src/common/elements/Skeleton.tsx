    ...
    texts.push(
      <div
        className={classNames(
          'h-2 bg-gray-600 rounded-full dark:bg-gray-700',
          lines === 1 ? '' : 'mb-3'
        )}
        key={i}
      />
...
const ImageSkeleton: FC<BaseProps> = ({
  children,
  isLoading,
  width,
  height,
  ...
  isLoading ? (
    ...
    >
      <div className="h-full bg-gray-500 rounded dark:bg-gray-700 relative">
        <div className="absolute flex items-center justify-center left-0 right-0 top-0 bottom-0">
          <svg
            className="w-12 h-12 text-gray-500 dark:text-gray-700"
            aria-hidden="true"
            fill="currentColor"
            viewBox="0 0 640 512"
          >
...
const Rectangle: FC<BaseProps> = ({ children, isLoading, width, height }) =>
  isLoading ? (
    ...
    >
      <div
        className={classNames(
          'flex items-center justify-center bg-gray-500 rounded dark:bg-gray-700 w-full h-full'
        )}
      >
...
const Avatar: FC = () => (
  <div className="w-full h-full">
    <svg
      className="text-gray-500 w-full h-full dark:text-gray-700"
      aria-hidden="true"
      fill="currentColor"
      viewBox="0 0 20 20"
...
const Skeleton: FC<SkeletonProps> = ({
  ...
  if (variant === 'image')
    return <ImageSkeleton {...props}>{children}</ImageSkeleton>
